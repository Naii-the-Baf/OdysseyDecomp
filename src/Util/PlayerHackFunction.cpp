...
// NON-MATCHING: https://decomp.me/scratch/1moRf
void endHackElectricWireToDestination(IUsePlayerHack** playerHack, const sead::Vector3f& targetPos,
    ...
    initHackEndParam(&hackEndParam, hackKeeper);
    hackEndParam.isDestination = true;
    sead::Vector3f vel = (posAfterHack - targetPos) * 0.04f;

    hackEndParam.vel.x = vel.x;
    hackEndParam.vel.y = 8.0f;
    hackEndParam.vel.z = vel.z;
...
void endHackFastenerToDestination(IUsePlayerHack** playerHack, const sead::Vector3f& start,
    ...
    sead::Vector3f vel;
    vel.y = (posAfterHack.y - start.y) * 0.05f;
    vel.x = (posAfterHack.x - start.x) * 0.05f;
    vel.z = (posAfterHack.z - start.z) * 0.05f;
...
void sendMsgHackerNoReaction(const IUsePlayerHack* playerHack, al::HitSensor* receiver,
                             al::HitSensor* sender) {
    sendMsgPlayerItemGetAll(receiver, sender);
}
